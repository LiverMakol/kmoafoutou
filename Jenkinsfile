    ...
    stages{
        ...
        stage('Build docker image') {
            steps {
                ...
                sh "printenv"
                sh "docker build -t livermakol/${BRANCH_NAME}_default_image ."
            }
        }
        ...
        stage('Test docker image') { 
            steps {
                sh "docker run -d -p 5012:8000 --name default_container_${BRANCH_NAME} livermakol/${BRANCH_NAME}_default_image"
            }
        }
        ...
        stage("Push image to Docker Hub"){
            steps{
                withCredentials([usernamePassword(credentialsId: 'DOCKER_CREDS_KMOAFOUTOU', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]){
                    ...
                    docker push livermakol/${BRANCH_NAME}_default_image:latest
                    """
                }
            }
        }
