pipeline{
    ...
    stages{
        ...
        stage('Test docker image') { 
            steps {
                sh "docker run -d -p 5012:8000 --name default_container_$env.BRANCH_NAME hervlokossou/$env.BRANCH_NAME/default_image_$env.BRANCH_NAME"
            }
        }
        ...
        stage("Push image to Docker Hub"){
            steps{
                withCredentials([usernamePassword(credentialsId: 'DOCKER_CREDS_KMOAFOUTOU', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]){
                    ...
                    docker push livermakol/${BRANCH_NAME}/default_image_$env.BRANCH_NAME:latest
                    """
                }
            }
        }
